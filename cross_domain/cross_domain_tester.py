...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output_path", type=str, default="./results/scores_new.xlsx", help="结果保存的Excel路径（可选）")
    parser.add_argument("--scatter_path", type=str, default="./results/score_scatter.png", help="散点图保存路径")
    parser.add_argument("--sources", type=str, nargs="+", default=["law"], help="筛选的来源域列表，如 law 等。如果不提供则使用全部数据")
    ...
    parser.add_argument("--ckpt_path", type=str, default="../checkpoints/shared_encoder_adapter_device_0.pth", help="编码器权重文件路径")
    ...
    print(f"Precision (machine) = {best_metrics['precision']*100:.2f}%, Recall (machine) = {best_metrics['recall']*100:.2f}%")
    ...
    plt.scatter(machine_x, machine_y, c='r', label='Machine-generated', alpha=0.7)
    plt.scatter(human_x, human_y, c='b', label='Human-written', alpha=0.7)
    ...
    plt.xlabel("Cross-Entropy (Observer vs Performer)")
    plt.ylabel("Perplexity (Performer Model)")
