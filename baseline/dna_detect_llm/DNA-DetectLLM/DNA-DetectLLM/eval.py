...
def load_json_texts_from_ch3_zh(mixed_json_path):
    human_texts = []
    machine_texts = []
    ...
    with open(mixed_json_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if "chatgpt_answers" in item:
                machine_texts.extend(item["chatgpt_answers"])
...
def main():
    ...
    parser.add_argument('--ch3_file', type=str, required=True, help="Path to mixed_text JSON file.")
    args = parser.parse_args()

    # human_texts, ai_texts = load_json_texts(args.human_file, args.ai_file)
    human_texts, ai_texts = load_json_texts_from_ch3_zh(args.ch3_file)
