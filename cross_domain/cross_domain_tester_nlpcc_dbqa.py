...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output_path", type=str, default="./results/scores_new_nlpcc_dbqa.xlsx", help="结果保存的Excel路径（可选）")
    parser.add_argument("--scatter_path", type=str, default="./results/score_scatter_nlpcc_dbqa.png", help="散点图保存路径")
    parser.add_argument("--sources", type=str, nargs="+", default=["nlpcc_dbqa"], help="筛选的来源域列表，如 law 等。如果不提供则使用全部数据")
    ...
    parser.add_argument("--ckpt_path", type=str, default="../checkpoints/shared_encoder_adapter_jsonl_nlpcc_dbqa.pth", help="编码器权重文件路径")
    ...
    with open(data_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not human_answers and not chatgpt_answers:
                continue
    ...
    ppl_values = []
    xppl_values = []
    valid_true_labels = []  # 用于存储有效的 true_labels
    ...
    for text in texts:
        ...
        # 确保只添加有效的标签
        if len(true_labels) > 0:  # 检查 true_labels 是否非空
            valid_true_labels.append(true_labels[len(valid_true_labels)])  # 与处理的样本对齐

    # 将原始分数保存到Excel（可选）
    df_scores = pd.DataFrame({"score": scores, "label": valid_true_labels})
    ...
    # 检查 scores 和 true_labels 是否有相同的长度
    if len(scores) != len(true_labels):
        # 如果长度不一致，打印警告并进行调整
        print(f"Warning: scores length = {len(scores)}, true_labels length = {len(true_labels)}")
        min_length = min(len(scores), len(true_labels))
        scores = scores[:min_length]  # 截取最小长度的部分
        true_labels = true_labels[:min_length]  # 截取最小长度的部分
...
# 新增功能 2: 固定阈值下的 F1 分数
given_thr = 1.964640  # 你可以修改为其他想要的硬编码阈值
